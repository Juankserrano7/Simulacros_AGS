"""Módulo de renderizado para el Ranking Global de estudiantes."""
...
def _slug(text: str) -> str:
    """Genera un slug alfanumérico seguro para nombres de columnas."""
    return re.sub(r"[^a-z0-9]+", "_", text.lower()).strip("_") or "sim"


def _dataframe_to_excel_table(df: pd.DataFrame, table_name: str, style_name: str = "TableStyleMedium9") -> bytes:
    """Exporta un DataFrame a un archivo Excel formateado como tabla oficial."""
...
def render(simulacros: List[dict], materias: List[str]):
    st.markdown("<h2 class='section-header'>🏆 Top Global</h2>", unsafe_allow_html=True)

    if not simulacros:
        st.warning("⚠️ No hay simulacros disponibles para generar el ranking.")
        return
    ...
    for sim in simulacros:
        ...
        base["ESTUDIANTE"] = base["ESTUDIANTE"].apply(normalize_student_name)
    ...
    if top_global.empty:
        ...
        for idx in range(len(top_global)):
            ...
            st.markdown(
                f"""
            <div style="display: flex; align-items: center; padding: 1rem; margin-bottom: 0.5rem; background: linear-gradient(90deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 10px;">
    ...
    with tab1:
        ...
        st.dataframe(tabla_estilo, use_container_width=True, height=600)
    ...
    with col_dl1:
        excel_data = _dataframe_to_excel_table(tabla_mostrar, "TablaRanking", "TableStyleMedium9")
        st.download_button(
    ...
    with col_dl2:
        excel_completo = _dataframe_to_excel_table(datos_unificados, "DatosCompletos", "TableStyleMedium2")
        st.download_button(
